import os
from sys import platform
import argparse

parser = argparse.ArgumentParser()
parser.add_argument("-r", "--release", help="Generate release build (debug by default).", action="store_true")
parser.add_argument("-lw", "--linux_to_win64", help="Cross compile to Windows.", action="store_true")
parser.add_argument("-a8", "--armv8", help="Cross compile to ARM v8.", action="store_true")
parser.add_argument("-c", "--clang", help="Use Clang instead of default.", action="store_true")
args = parser.parse_args()

build_type = 'Release' if args.release else 'Debug'

source = os.path.dirname(os.path.realpath(__file__))

def create_symlink(src, dst):
    # create all parent directories of the symlink one
    parent_dir = os.path.dirname(dst)
    os.makedirs(parent_dir, exist_ok=True)

    try:
        os.symlink(src, dst)
    except:
        pass

def build(source, build_type, symlinks = [], symlink_pairs = []):
    cross_compile_dir = ''
    premake_args = ''
    if args.linux_to_win64:
        cross_compile_dir = 'mingw-'
        premake_args = ' --mingw'
    elif args.armv8:
        cross_compile_dir = 'armv8-'
        premake_args = ' --armv8'
    elif args.clang:
        conan_profile = ' -pr=clang'
        premake_args = ' --cc=clang'

    build_dir = '../../example-build/' + cross_compile_dir + build_type + '/'
    gui_dir = '../../build/' + cross_compile_dir + build_type + '/'

    # create build directory
    os.makedirs(build_dir, exist_ok=True)
    os.chdir(build_dir)

    # symlinks
    for path in symlinks:
        create_symlink(source + '/' + path, './' + path)

    for src_path, dst_path in symlink_pairs:
        create_symlink(source + '/' + src_path, './' + dst_path)

    # choose premake generator based on OS
    os.chdir(source)

    def premake_generate(generator):
        os.system('premake5 ' + generator + ' --location="' + build_dir + '" --gui-path="' + gui_dir + '"' + premake_args)

    if platform == 'win32':
        premake_generate('vs2019')
    else:
        premake_generate('gmake')
        premake_generate('vscode')
        premake_generate('export-compile-commands')


build(source, build_type)
